...
filter_coverage_age <- function(data, age_group) {
  if (
    is.null(age_group) ||
      length(age_group) != 1 ||
      is.na(age_group) ||
      !age_group %in% names(coverage_age_ranges)
  ) {
    age_group <- "2_59"
  }
...
server <- function(input, output, session) {
  input_or <- function(value, default) {
  ...
  default_community_id <- community %>%
    arrange(county) %>%
    slice(1) %>%
    pull(community_id)

  selected_community_id <- reactive({
    input_or(input$community_id, default_community_id)
  })
  ...
  coverage_scope <- reactive({
    ...
    coverage_jurisdiction <- input_or(
      input$coverage_jurisdiction,
      "PALAU"
    )
    
    if (!identical(coverage_jurisdiction, "PALAU")) {
      x <- x %>%
        filter(county == coverage_jurisdiction)
    }
  ...
  coverage_age_data <- reactive({
    filter_coverage_age(
      coverage_scope(),
      input_or(input$coverage_age_group, "2_59")
    )
  })
  ...
  observeEvent(
    ...
    {
      coverage_age_group <- input_or(
        input$coverage_age_group,
        "2_59"
      )
      choices <- coverage_indicator_choices
      
      if (coverage_age_group %in% c("2_3", "4_5", "6_11")) {
        choices <- choices[names(choices) != "MMR UTD"]
      }
      
      selected <- input_or(
        isolate(input$coverage_map_indicator),
        "coverage_dtap"
      )
  ...
  output$coverage_definition_note <- renderUI({
    ...
    tagList(
      tags$p(
        ...
        coverage_definition_notes[[coverage_age_group]]
      ),
      if (coverage_age_group %in% c("2_59", "2_83")) {
        tags$p(
          class = "small-note coverage-definition-note",
  ...
  coverage_map_data <- reactive({
    indicator_variable <- input_or(
      input$coverage_map_indicator,
      "coverage_dtap"
    )
  ...
  output$vaccination_coverage_map <- renderLeaflet({
    ...
    indicator_label <- names(coverage_indicator_choices)[
      match(indicator_variable, coverage_indicator_choices)
    ]
  ...
  filtered <- reactive({
    min_priority <- input_or(input$min_priority, 0)
    x <- community %>%
      filter(is.na(priority_score) | priority_score >= min_priority)
  ...
  selected_comm <- reactive({
    community %>%
      filter(community_id == selected_community_id()) %>%
      slice(1)
  })
  ...
  plan <- eventReactive(input$optimize, {
    selected_counties <- input_or(
      input$plan_communities,
      character()
    )
    stops <- community %>%
      filter(community_id %in% selected_counties)

    nearest_neighbor_route(
      stops,
      input_or(input$hub_lon, 134.477167975299),
      input_or(input$hub_lat, 7.34027834639506),
      input_or(input$speed, 25),
      input_or(input$service, 4),
      input_or(input$setup, 30),
      input_or(input$workday, 8),
      input_or(input$teams, 1)
    )
  ...
  output$route_map <- renderLeaflet({
    it <- plan()$itinerary
    hub_lon <- input_or(input$hub_lon, 134.477167975299)
    hub_lat <- input_or(input$hub_lat, 7.34027834639506)

    m <- leaflet() %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addMarkers(hub_lon, hub_lat, label = "Departure hub")

    if (nrow(it)) {
      for (tm in unique(it$team)) {
        z <- it[it$team == tm, ]
        m <- m %>%
          addPolylines(
            lng = c(hub_lon, z$longitude),
            lat = c(hub_lat, z$latitude),
            weight = 3,
            label = paste("Team", tm)
          ) %>%
          addCircleMarkers(
            ...
            radius = 7,
            label = ~paste0(
              "Team ", team, " stop ", stop_sequence, ": ", county
            ),
            popup = ~paste0(
              "<b>", county, "</b><br>",
              "Children targeted: ", children_not_utd, "<br>",
              "Day: ", day
            )
          )
      }
  ...
  output$plan_product_table <- renderDT({
    ...
    product_table_data <- product_needs %>%
      filter(
        community_id %in% input_or(input$plan_communities, character())
      ) %>%
      group_by(product) %>%
      summarise(
  ...
  output$scenario_table <- renderDT({
    scenario_distance <- input_or(input$scen_distance, 250)
    scenario_counties <- input_or(input$scen_communities, 8)
    scenario_children <- input_or(input$scen_children, 300)
    ...
    scenarios <- data.frame(
      ...
      mutate(
        Estimated_travel_hours = round(scenario_distance / Speed_kmh, 1),
        Estimated_service_hours = round(scenario_counties * .5 + scenario_children * 4 / 60, 1),
        Estimated_team_days = ceiling((Estimated_travel_hours + Estimated_service_hours) / (Teams * Workday_hours)),
        Children_per_team_day = round(scenario_children / Estimated_team_days / Teams)
      ) %>%
      transmute(
        `Scenario` = Scenario,
        `Teams` = Teams,
